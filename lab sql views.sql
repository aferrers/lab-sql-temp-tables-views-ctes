...
CREATE TEMPORARY TABLE rental_pmt_info as (
	...
	FROM
			payment p
	RIGHT JOIN rental_info ro ON p.customer_id = ro.customer_id
...
(
  SELECT 
	*, 
    round((total_paid/total_number_of_rental),2) as average_payment_per_rental 
FROM 
	rental_pmt_info
)
SELECT * FROM CTE_customer_rental_info
WHERE customer_id = 1;
